...
{
    {
        ...
        else if constexpr (std::is_same_v<T, PizzeriaCookerIntern>)
        {
            _interns.insert(std::make_pair(worker, false));
            _free_cookers++;
        }
        else if constexpr (std::is_same_v<T, Cashier>)
        {
            _staff.insert(std::make_pair(worker, 0));
        }
        else if constexpr (std::is_same_v<T, CallCenterOperator>)
        {
            _staff.insert(std::make_pair(worker, 0));
        }
        else if constexpr (std::is_same_v<T, PizzeriaManager>)
        {
            _staff.insert(std::make_pair(worker, 0));
        }
        else if constexpr (std::is_same_v<T, Accountant>)
        {
            _staff.insert(std::make_pair(worker, 0));
        }
        else if constexpr (std::is_same_v<T, Hostess>)
        {
            _staff.insert(std::make_pair(worker, 0));
        }
        else if constexpr (std::is_same_v<T, Courier>)
        {
            _staff.insert(std::make_pair(worker, 0));
        }
        else
        {
            static_assert(std::is_same_v<T, PizzeriaWorker>, "Unsupported employee type");
        }
        return worker;
    }
}
...
template std::shared_ptr<Cashier> PizzaDepartment::Hire<Cashier>(const Human &);
template std::shared_ptr<CallCenterOperator> PizzaDepartment::Hire<CallCenterOperator>(const Human &);
template std::shared_ptr<PizzeriaManager> PizzaDepartment::Hire<PizzeriaManager>(const Human &);
template std::shared_ptr<Accountant> PizzaDepartment::Hire<Accountant>(const Human &);
template std::shared_ptr<Hostess> PizzaDepartment::Hire<Hostess>(const Human &);
template std::shared_ptr<Courier> PizzaDepartment::Hire<Courier>(const Human &);
...
int PizzaDepartment::GetStaffCount()
{
    std::lock_guard<std::mutex> lock(mtx);
    return _staff.size();
}